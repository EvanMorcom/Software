...
std::pair<Angle, Point> calcOpenDirection(Point origin, Segment segment, std::vector<Circle> obstacles) {
    ...
    if(obstacles.size() == 0){
        return std::make_pair(((segment.getSegStart() - origin).orientation() - (segment.getEnd() - origin).orientation()).abs(), (segment.getSegStart() + segment.getEnd().toVector()/2));
    }

    for(Circle circle : obstacles){
        // If the reference is inside an obstacle there is no open direction
        if(contains(circle, origin)){
            return std::make_pair(Angle::fromDegrees(0), (segment.getSegStart() + segment.getEnd().toVector()/2));
        }
    ...
    if(obstacle_segment_projections.size() == 0){
        return std::make_pair((segment.getSegStart() - origin).orientation() - (segment.getEnd() - origin).orientation().abs(), Point( segment.getSegStart() + segment.getEnd().toVector()/2));
    }
