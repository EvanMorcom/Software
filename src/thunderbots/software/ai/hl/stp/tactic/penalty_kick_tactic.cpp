...
void PenaltyKickTactic::calculateNextIntent(IntentCoroutine::push_type& yield)
{
    ...
    MoveAction approach_ball_move_act = MoveAction(MoveAction::ROBOT_CLOSE_TO_DEST_THRESHOLD, MoveAction::ROBOT_CLOSE_TO_ORIENTATION_THRESHOLD, false);
    MoveAction rotate_with_ball_move_act = MoveAction(MoveAction::ROBOT_CLOSE_TO_DEST_THRESHOLD, MoveAction::ROBOT_CLOSE_TO_ORIENTATION_THRESHOLD, false);
    ...
    while(true) {
        yield(approach_ball_move_act.updateStateAndGetNextIntent(
                *robot, robot->position(), robot->orientation(), 0, true));
    }
